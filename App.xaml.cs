...
using LilySwapper.Workspace;

namespace LilySwapper;
...
public partial class App : Application
{
    ...
    public static readonly string[] SubDirectories =
        { $"{Config}\\DLLS", $"{Config}\\Plugins", $"{Config}\\LOGS", $"{Config}\\Binaries" };

    protected override void OnStartup(StartupEventArgs e)
    {
        var currentprocess = Process.GetProcessesByName(Process.GetCurrentProcess().ProcessName);
        if (currentprocess.Length > 1)
        {
            Win32.SetProcessHigher(currentprocess);
            Message.DisplaySTA("Info",
                "It has been detected that Galaxy Swapper is already running. The process will be brought to the front.\nIf there is an issue, please end the 'Galaxy Swapper v2' process in Task Manager and restart the application.");
            Environment.Exit(0);
        }
        ...
        foreach (var sub in SubDirectories) Directory.CreateDirectory(sub);

        Output.Initialize(); //Serilog logger

        Log.Information("Version {0}", Global.Version);
        Log.Information("API-Version {0}", ApiVersion);
