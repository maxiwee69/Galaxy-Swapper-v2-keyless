...
public static class UEFN
{
    ...
    public static void DownloadMain(string paks, string tag)
    {
        ...
        var cacheDownloadable =
            Cache["Downloadables"].OfType<JObject>().FirstOrDefault(d => d.Value<string>("tag") == tag);
        ...
        if (downloadable["zip"].KeyIsNullOrEmpty())
            downloadables.Add(new Downloadable
            {
            ...
            downloadables.Add(new Downloadable { Zip = downloadable["zip"].Value<string>() });

        Log.Information($"Downloading {tag} downloadable");
        Download(new DirectoryInfo(paks), downloadables, out var usedslots);
    ...
    public static void Add(string paks, string name, Downloadable downloadable)
    {
        ...
        Download(new DirectoryInfo(paks), new List<Downloadable> { downloadable }, out var usedslots);
    ...
    private static void Download(DirectoryInfo paks, List<Downloadable> downloadables, out List<string> usedslots)
    {
        var temp = new DirectoryInfo(
            $"{Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData)}\\Temp\\GalaxyDownloadables");
        usedslots = new List<string>();

        if (temp.Exists) Directory.Delete(temp.FullName, true);
        ...
        foreach (var slot in openSlots.Take(downloadables.Count))
        {
        ...
        foreach (var downloadable in downloadables)
        {
            ...
            {
                var zippedpath = $"{temp}\\Zipped ({openSlots.First()})";
                ...
                Misc.Download(gamefiles.FullName, downloadable.Zip, "UEFN zipped game file");

                if (!gamefiles.Exists) throw new CustomException($"{gamefiles.FullName} does not exist? How?");
                ...
                {
                    Log.Error(
                        $"Drive: {tempdriveInfo.Name} does not have enough space for UEFN game files! Required: {gamefiles.Length} Has: {tempdriveInfo.TotalFreeSpace}");
                    throw new CustomException(
                        $"Drive: {tempdriveInfo.Name} does not have enough space for UEFN game files!\nRequired: {gamefiles.Length}\nHas: {tempdriveInfo.TotalFreeSpace}\nPlease make room on the {tempdriveInfo.Name} drive for this process to continue.");
                }

                Log.Information("Extracting GameFiles.zip");
                ZipFile.ExtractToDirectory(gamefiles.FullName, zippedpath);

                var ucas = Misc.FindFileByExtension(zippedpath, ".ucas");
                if (ucas is null) throw new CustomException("Zipped UEFN game files does not contain a .ucas file!");

                File.Move(ucas, $"{temp}\\{openSlots.First()}.ucas");
                Log.Information($"Moved: {ucas} to: {temp}\\{openSlots.First()}.ucas");

                var utoc = Misc.FindFileByExtension(zippedpath, ".utoc");
                if (utoc is null) throw new CustomException("Zipped UEFN game files does not contain a .utoc file!");
                ...
                var pak = Misc.FindFileByExtension(zippedpath, ".pak");
                if (pak is null) throw new CustomException("Zipped UEFN game files does not contain a .pak file!");

                File.Move(pak, $"{temp}\\{openSlots.First()}.pak");
                Log.Information($"Moved: {pak} to: {temp}\\{openSlots.First()}.pak");

                var sig = Misc.FindFileByExtension(zippedpath, ".sig");
                if (sig is null) throw new CustomException("Zipped UEFN game files does not contain a .sig file!");
            ...
            {
                Misc.Download($"{temp}\\{openSlots.First()}.ucas", downloadable.Ucas, "UEFN game file (Ucas)");
                Misc.Download($"{temp}\\{openSlots.First()}.utoc", downloadable.Utoc, "UEFN game file (Utoc)");
                Misc.Download($"{temp}\\{openSlots.First()}.pak", downloadable.Pak, "UEFN game file (Pak)");
                Misc.Download($"{temp}\\{openSlots.First()}.sig", downloadable.Sig, "UEFN game file (Sig)");
                Misc.Download($"{temp}\\{openSlots.First()}.backup", downloadable.Utoc, "UEFN game file (Utoc)");
        ...
        long requriedSize = 0;
        foreach (var downloaded in temp.GetFiles()) requriedSize += downloaded.Length;

        var driveInfo = new DriveInfo(paks.Root.Name);
        if (requriedSize > driveInfo.TotalFreeSpace)
        {
            Log.Error(
                $"Drive: {driveInfo.Name} does not have enough space for UEFN game files! Required: {requriedSize} Has: {driveInfo.TotalFreeSpace}");
            throw new CustomException(
                $"Drive: {driveInfo.Name} does not have enough space for UEFN game files!\nRequired: {requriedSize}\nHas: {driveInfo.TotalFreeSpace}\nPlease make room on the {driveInfo.Name} drive for this process to continue.");
        }
    ...
    private static List<string> FindOpenSlots(string paks)
    {
        ...
        foreach (string slot in parse["Slots"])
            if (!File.Exists($"{paks}\\{slot}.ucas") || !File.Exists($"{paks}\\{slot}.utoc") ||
                !File.Exists($"{paks}\\{slot}.pak") || !File.Exists($"{paks}\\{slot}.sig"))
            {
