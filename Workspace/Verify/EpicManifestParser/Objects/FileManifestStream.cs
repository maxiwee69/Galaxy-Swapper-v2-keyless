...
public class FileManifestStream : Stream
{
    ...
    internal FileManifestStream(FileManifest fileManifest, Manifest manifest)
    {
        ...
        foreach (var chunkPart in fileManifest.ChunkParts) Length += chunkPart.Size;
    ...
    {
        if (destination == null || destination == Null || !destination.CanWrite) return;
        ...
        foreach (var fileChunkPart in _fileChunkParts)
        {
            ...
            await chunkStream.ReadAsync(buffer.AsMemory(0, fileChunkPart.Size), cancellationToken)
                .ConfigureAwait(false);
            await destination.WriteAsync(new ReadOnlyMemory<byte>(buffer, 0, fileChunkPart.Size), cancellationToken)
                .ConfigureAwait(false);
    ...
    private async Task<Stream> GetChunkStreamAsync(FileChunk chunk, CancellationToken cancellationToken)
    {
        ...
        if (cachePath == null) return outStream;

        {
            await using var cacheFs = new FileStream(cachePath, FileMode.OpenOrCreate, FileAccess.ReadWrite,
                FileShare.ReadWrite);
    ...
    public async Task<int> ReadAtAsync(long position, byte[] buffer, int offset, int count,
        CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            {
                await chunkStream.ReadAsync(buffer.AsMemory(bytesRead + offset, bytesLeft), cancellationToken)
                    .ConfigureAwait(false);
                bytesRead += bytesLeft;
                break;
            }

            await chunkStream.ReadAsync(buffer.AsMemory(bytesRead + offset, chunkBytes), cancellationToken)
                .ConfigureAwait(false);
            bytesRead += chunkBytes;
            startPos = 0;

            if (++i == _fileChunkParts.Count) break;
        }
