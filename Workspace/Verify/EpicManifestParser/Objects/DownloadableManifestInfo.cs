...
public abstract class DownloadableManifestInfo
{
    ...
    public byte[] DownloadManifestData(string cacheDir = null)
    {
        ...
        if (path != null && File.Exists(path)) return File.ReadAllBytes(path);
        ...
        if (path != null && data != null) File.WriteAllBytes(path, data);
    ...
    {
        var path = cacheDir == null ? null : Path.Combine(cacheDir, FileName);
        if (path != null && File.Exists(path)) return await File.ReadAllBytesAsync(path).ConfigureAwait(false);
        ...
        if (path != null && data != null) await File.WriteAllBytesAsync(path, data).ConfigureAwait(false);
