...
using GenericReader;
using Ionic.Zlib;
using LilySwapper.Workspace.Verify.EpicManifestParser.Enums;
...
public sealed class Manifest
{
    ...
    public Manifest(byte[] data, ManifestOptions options = null)
    {
        ...
        if (options.ChunkCacheDirectory is { Exists: false }) options.ChunkCacheDirectory.Create();
        ...
        {
            var chunk = new FileChunk(guid, size, ChunkHashes[guid], ChunkShas[guid], DataGroups[guid],
                options.ChunkBaseUri);
    ...
    {
        if (!(Options.ChunkCacheDirectory is { Exists: true })) return default;
        ...
        {
            if (chunkMap.Contains(chunk.Name)) continue;
    ...
    private void ParseData(byte[] buffer)
    {
        ...
        if (dataVersion >= EManifestMetaVersion.Original)
        {
            ...
            for (var i = 0; i < count; i++) // Guid
            {
                var position = i * 4;
                var guid =
                    $"{guidValues[position]:X8}{guidValues[position + 1]:X8}{guidValues[position + 2]:X8}{guidValues[position + 3]:X8}";
                guids.Add(guid);
            }
        ...
        if (dataVersion >= EManifestMetaVersion.Original)
        {
            ...
            for (var i = 0; i < count; i++) CustomFields.Add(keys[i], values[i]);
        }
    }
}