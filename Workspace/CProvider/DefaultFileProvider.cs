...
public class DefaultFileProvider
{
    ...
    public DefaultFileProvider(DirectoryInfo pakDirectoryInfo)
    {
        ...
        IoStoreReaders = new List<IoStoreReader>();
        Keys = new Dictionary<FGuid, FAesKey>();
    }
    ...
    public void Initialize(List<string> blacklisted = null!)
    {
        ...
        {
            if (!ioFileInfo.Exists || ioFileInfo.Name.ToUpper() == "GLOBAL.BACKUP" || (blacklisted is not null &&
                    blacklisted.Contains(ioFileInfo.Name.SubstringBefore('.')))) continue;
        ...
        Log.Information(
            $"Mounted {IoStoreReaders.Count} successfully and failed to mount {failed} in {stopwatch.GetElaspedAndStop().ToString("mm':'ss")}");
    }
    ...
    public List<string> ListFiles()
    {
        ...
        foreach (var reader in IoStoreReaders) list.AddRange(reader.Files.Keys);
    ...
    public string FindGameFile(string path)
    {
        ...
        {
            var formatted =
                reader.Files.Keys.FirstOrDefault(gamepath =>
                    gamepath.SubstringBeforeLast('.').ToLower().EndsWith(path))!;
