...
public class IoStoreReader
{
    ...
    public bool Initialize()
    {
        ...
        if (IoStoreTocHeader.ContainerFlags.HasFlag(EIoContainerFlags.Encrypted))
        {
            ...
            {
                reader = new Reader(
                    FIoStoreTocResource.DirectoryIndexBuffer.Decrypt(
                        DefaultFileProvider.Keys[IoStoreTocHeader.EncryptionKeyGuid]));
            }
            else
            {
        ...
        Partitions = new Dictionary<int, string> { { 0, $"{IoFileInfo.Name.SubstringBefore('.')}" } };

        if (IoStoreTocHeader.PartitionCount > 1)
            for (var i = 1; i <= IoStoreTocHeader.PartitionCount; i++)
            {
                var partition = $"{IoFileInfo.Name.SubstringBefore('.')}_s{i}";
    ...
    private bool ReadMountPoint(Reader reader)
    {
        ...
        var badMountPoint = !MountPoint.StartsWith("../../..") && !MountPoint.StartsWith(",./../..");
    ...
    private bool ReadIndex(string directoryname, uint dir, ref FIoDirectoryIndexEntry[] directoryEntries,
        ref FIoFileIndexEntry[] fileEntries, ref string[] stringTable)
    {
        ...
        while (dir != invalidHandle)
        {
            ...
            var subDirectoryName = dirEntry.Name == invalidHandle
                ? directoryname
                : $"{directoryname}{stringTable[dirEntry.Name]}/";
            ...
            while (file != invalidHandle)
            {
                ...
                var gamefile = new GameFile(path, fileEntry.UserData,
                    FIoStoreTocResource.ChunkOffsetLengths[fileEntry.UserData], IoStoreTocHeader);

                Files[path] = gamefile;
                file = fileEntry.NextFileEntry;
            }

            ReadIndex(subDirectoryName, dirEntry.FirstChildEntry, ref directoryEntries, ref fileEntries,
                ref stringTable);
    ...
    public bool Export(GameFile gamefile, string path, long offset)
    {
        ...
        var uncompressedbuffer = new byte[compressionBlock.UncompressedSize];
        ...
        if (IoStoreTocHeader.ContainerFlags.HasFlag(EIoContainerFlags.Encrypted))
        {
            ...
            if (DefaultFileProvider.Keys.ContainsKey(IoStoreTocHeader.EncryptionKeyGuid))
                compressedbuffer =
                    compressedbuffer.Decrypt(DefaultFileProvider.Keys[IoStoreTocHeader.EncryptionKeyGuid]);
        }
        ...
        else
        {
            ...
            {
                Log.Error(Exception,
                    $"Failed to decompress {gamefile.Path} with compression method {compressionmethod}");
                return false;
            }
        }
