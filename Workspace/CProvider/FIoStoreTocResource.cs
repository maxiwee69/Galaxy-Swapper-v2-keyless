...
public class FIoStoreTocResource
{
    ...
    public FIoStoreTocResource(Reader reader, FIoStoreTocHeader header)
    {
        ...
        for (var i = 0; i < header.EntryCount; i++) ChunkOffsetLengths[i] = new FIoOffsetAndLength(reader);
        ...
        for (var i = 0; i < header.CompressedBlockEntryCount; i++)
        ...
        unsafe
        {
            ...
            {
                var name = new string((sbyte*)buffer + i * header.CompressionMethodNameLength, 0,
                    (int)header.CompressionMethodNameLength).TrimEnd('\0');
        ...
        if (header.Version >= EIoStoreTocVersion.DirectoryIndex &&
            ...
            header.DirectoryIndexSize > 0) DirectoryIndexBuffer = reader.ReadBytes((int)header.DirectoryIndexSize);
