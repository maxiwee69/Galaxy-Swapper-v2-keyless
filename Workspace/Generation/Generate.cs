using LilySwapper.Workspace.Generation.Types;

namespace LilySwapper.Workspace.Generation;
...
public static class Generate
{
    ...
    public static Dictionary<Type, Frontend> Cache = new();
    ...
    public static Dictionary<string, Cosmetic> Frontend(Type Type)
    {
        if (!Cache.ContainsKey(Type))
        {
            ...
            if (parseStats is null)
                ...
                stats = parseStats[Type.ToString()]
                    .ToDictionary(c => c["key"].Value<string>(), c => c["count"].Value<int>());
            ...
            var HideNsfw = Settings.Read(Settings.Type.HideNsfw).Value<bool>();
            ...
            if (Array.Count() != 0)
                foreach (var Cosmetic in Array)
                {
                    if (Cosmetic["Frontend"].Value<bool>())
                    {
                        ...
                        {
                            Name = Cosmetic["Name"].Value<string>(), ID = Cosmetic["ID"].Value<string>(),
                            Parse = Cosmetic, Type = Type
                        };
                        ...
                        if (Cosmetic["Downloadables"] != null)
                            foreach (var downloadable in Cosmetic["Downloadables"])
                                if (!downloadable["pak"].KeyIsNullOrEmpty() &&
                                    ...
                                    NewCosmetic.Downloadables.Add(new Downloadable
                                    {
                        ...
                        var key = $"{NewCosmetic.Name}:{NewCosmetic.ID}";

                        if (stats.ContainsKey(key)) NewCosmetic.Stats = stats[key];
            ...
            if (Settings.Read(Settings.Type.SortByStats).Value<bool>())
                NewCache.Cosmetics = NewCache.Cosmetics.OrderByDescending(pair => pair.Value.Stats)
    ...
    public static void AddMaterialOverridesArray(JToken parse, Asset asset)
    {
        ...
        {
            asset.MaterialData = new MaterialData();
            ...
            foreach (var material in parse["MaterialOverridesArray"]["Materials"])
                asset.MaterialData.Materials.Add(new OverrideMaterial
                {
                    Material = material["OverrideMaterial"].Value<string>(),
                    MaterialOverrideIndex = material["MaterialOverrideIndex"].Value<int>()
                });

            asset.MaterialData.MaterialOverrideFlags = new MaterialOverrideFlagsData
            {
                MaterialOverrideFlags = parse["MaterialOverridesArray"]["MaterialOverrideFlags"]["Index"].Value<int>()
            };

            if (parse["MaterialOverridesArray"]["MaterialOverrideFlags"]["Search"].KeyIsNullOrEmpty())
                asset.MaterialData.MaterialOverrideFlags.Offset =
                    parse["MaterialOverridesArray"]["MaterialOverrideFlags"]["Offset"].Value<long>();
            else
                asset.MaterialData.MaterialOverrideFlags.SearchBuffer =
                    parse["MaterialOverridesArray"]["MaterialOverrideFlags"]["Search"].Value<string>().HexToByte();
        }
    }
    ...
    public static void AddTextureParametersArray(JToken parse, Asset asset)
    {
        ...
        {
            asset.TextureData = new TextureData();
            ...
            foreach (var texture in parse["TextureParametersArray"]["Textures"])
                asset.TextureData.TextureParameters.Add(new TextureParameter
                {
                    TextureOverride = texture["TextureOverride"].Value<string>(),
                    TextureParameterNameForMaterial = texture["TextureParameterNameForMaterial"].Value<string>(),
                    MaterialIndexForTextureParameter = texture["MaterialIndexForTextureParameter"].Value<int>()
                });
        }
    }
