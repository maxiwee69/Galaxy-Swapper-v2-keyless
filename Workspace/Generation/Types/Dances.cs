...
public static class Dances
{
    public static void Format(Cosmetic Cosmetic, List<Option> Options)
    {
        ...
        if (Parse["OverrideOptions"] != null && (Parse["OverrideOptions"] as JArray).Any())
            foreach (var Override in Parse["OverrideOptions"])
                switch (Override["Type"].Value<string>())
                {
                    case "Exception":
                    {
                        var OverrideCosmetic = Generate.Cache[Generate.Type.Gliders]
                            .Cosmetics[Override["CacheKey"].Value<string>()];
        ...
        var Animation = (Parse["Animations"] as JArray).ToObject<List<string>>().OrderBy(s => s.Length)
            .FirstOrDefault();
        ...
        foreach (var Option in Options.Concat(OverrideOptions))
        {
            ...
            if (Parse["Loop"].Value<bool>() != OParse["Loop"].Value<bool>() ||
                Parse["Type"].Value<string>() != OParse["Type"].Value<string>())
                continue;
            ...
            var Strings = OParse["Strings"].ToDictionary(obj => obj["Property"].Value<string>(),
                obj => obj["AssetPathName"].Value<string>());

            foreach (var String in Parse["Strings"])
            {
                var Property = String["Property"].Value<string>();
            ...
            if (Parse["Additional"] != null)
                ...
                {
                    var NewAdditionalAsset = new Asset
