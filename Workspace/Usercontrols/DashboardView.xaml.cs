...
public partial class DashboardView : UserControl
{
    ...
    {
        Memory.MainView.SetOverlay(Memory.NotesView);
    }
    ...
    private void DashboardView_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        Storyboard = Interface.SetThicknessAnimations(new Interface.ThicknessAnim
        {
            ...
            ElementAnim = new ThicknessAnimation
                { To = new Thickness(0, 0, 12, 7), Duration = new TimeSpan(0, 0, 0, 0, 200) }
        });
        Storyboard2 = Interface.SetThicknessAnimations(new Interface.ThicknessAnim
        {
            Element = ReadMore,
            ElementAnim = new ThicknessAnimation
                { To = new Thickness(0, 0, 0, 7), Duration = new TimeSpan(0, 0, 0, 0, 200) }
        });

        Storyboard2.Completed += delegate { ReadMore.Visibility = Visibility.Hidden; };

        var versionParse = Endpoint.Read(Endpoint.Type.Version);

        if (!versionParse[Global.Version]["Banner"].KeyIsNullOrEmpty())
            BannerImage.LoadImage(versionParse[Global.Version]["Banner"].Value<string>(),
                "/WorkSpace/Assets/Banner.png");
