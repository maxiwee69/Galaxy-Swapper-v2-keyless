...
public partial class SettingsView : UserControl
{
    ...
    public void SettingsView_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        SwapLogsDescription.Text =
            string.Format(Languages.Read(Languages.Type.View, "SettingsView", "SwapLogsDescription"), Count, Ucas,
                Utoc);
        ...
        RefreshCosmeticsDescription.Text =
            Languages.Read(Languages.Type.View, "SettingsView", "RefreshCosmeticsDescription");
        ...
        DiscordRichPresenceHeader.Text =
            Languages.Read(Languages.Type.View, "SettingsView", "DiscordRichPresenceHeader");
        DiscordRichPresenceDescription.Text =
            Languages.Read(Languages.Type.View, "SettingsView", "DiscordRichPresenceDescription");
        AutoCloseFortniteHeader.Text = Languages.Read(Languages.Type.View, "SettingsView", "AutoCloseFortniteHeader");
        AutoCloseFortniteDescription.Text =
            Languages.Read(Languages.Type.View, "SettingsView", "AutoCloseFortniteDescription");
        ...
        BackPackGenderDescription.Text =
            Languages.Read(Languages.Type.View, "SettingsView", "BackPackGenderDescription");
        ...
        HeroDefinitionDescription.Text =
            Languages.Read(Languages.Type.View, "SettingsView", "HeroDefinitionDescription");
    ...
    private void ShowConverted_Click(object sender, RoutedEventArgs e)
    {
        Memory.MainView.SetOverlay(new SwappedView());
    }
    ...
    private void EditPath_Click(object sender, RoutedEventArgs e)
    {
        using (var dialog = new CommonOpenFileDialog
               {
                   IsFolderPicker = true, Title = Languages.Read(Languages.Type.View, "SettingsView", "EditPathTip")
               })
        {
            if (dialog.ShowDialog() == CommonFileDialogResult.Ok)
            {
                ...
                if (!Directory.Exists($"{selected}\\FortniteGame\\Content\\Paks"))
                {
                    Message.DisplaySTA(Languages.Read(Languages.Type.Header, "Error"),
                        Languages.Read(Languages.Type.Message, "InvalidInstallationSelection"));
                    return;
                }
    ...
    private void EpicEditPath_Click(object sender, RoutedEventArgs e)
    {
        using (var dialog = new CommonOpenFileDialog
               {
                   IsFolderPicker = true, Title = Languages.Read(Languages.Type.View, "SettingsView", "EpicEditPathTip")
               })
        {
            if (dialog.ShowDialog() == CommonFileDialogResult.Ok)
            {
                ...
                if (!Directory.Exists($"{selected}\\Epic Games\\Launcher\\Portal\\Binaries\\Win64") ||
                    !File.Exists($"{selected}\\Epic Games\\Launcher\\Portal\\Binaries\\Win64\\EpicGamesLauncher.exe"))
                {
                    Message.DisplaySTA(Languages.Read(Languages.Type.Header, "Error"),
                        Languages.Read(Languages.Type.Message, "InvalidEpicInstallationSelection"));
                    return;
                }

                Settings.Edit(Settings.Type.EpicInstalltion,
                    $"{selected}\\Epic Games\\Launcher\\Portal\\Binaries\\Win64\\EpicGamesLauncher.exe");
                EpicInstallationDescription.Text = Settings.Read(Settings.Type.EpicInstalltion).Value<string>();
            }
        }
    }
    ...
    private void Verify_Click(object sender, RoutedEventArgs e)
    {
        ...
        var paks = $"{Settings.Read(Settings.Type.Installtion).Value<string>()}\\FortniteGame\\Content\\Paks";
    ...
    {
        if (Message.DisplaySTA(Languages.Read(Languages.Type.Header, "Question"),
                Languages.Read(Languages.Type.Message, "ResetSwaps"), MessageBoxButton.YesNo) == MessageBoxResult.Yes)
        {
            SwapLogs.Clear();
            SwapLogsDescription.Text =
                string.Format(Languages.Read(Languages.Type.View, "SettingsView", "SwapLogsDescription"), 0, 0, 0);
        }
    }
    ...
    {
        if (Message.DisplaySTA(Languages.Read(Languages.Type.Header, "Question"),
                Languages.Read(Languages.Type.Message, "RevertAllCosmetics"), MessageBoxButton.YesNo) ==
            MessageBoxResult.Yes)
        {
            EpicGamesLauncher.Close();

            var pakDirectoryInfo =
                new DirectoryInfo(
                    $"{Settings.Read(Settings.Type.Installtion).Value<string>()}\\FortniteGame\\Content\\Paks");
            var epicinstallation = Settings.Read(Settings.Type.EpicInstalltion).Value<string>();
            ...
            foreach (var backupIoFileInfo in
                     pakDirectoryInfo.EnumerateFiles("*.backup*", SearchOption.TopDirectoryOnly))
            {
    ...
    private void CloseFortnite_Click(object sender, RoutedEventArgs e)
    {
        ...
        Message.DisplaySTA(Languages.Read(Languages.Type.Header, "Info"),
            Languages.Read(Languages.Type.Message, "CloseFortnite"));
    }
    ...
    private void AutoCloseFortnite_Click(object sender, RoutedEventArgs e)
    {
        Settings.Edit(Settings.Type.CloseFortnite, AutoCloseFortnite.IsChecked);
    }

    private void KickWarning_Click(object sender, RoutedEventArgs e)
    {
        Settings.Edit(Settings.Type.KickWarning, KickWarning.IsChecked);
    }

    private void ShareStats_Click(object sender, RoutedEventArgs e)
    {
        Settings.Edit(Settings.Type.ShareStats, ShareStats.IsChecked);
    }
    ...
    private void BackPackGender_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (Generate.Cache.ContainsKey(Generate.Type.Backpacks))
            Generate.Cache[Generate.Type.Backpacks]?.Cosmetics.Values.Where(cosmetic => cosmetic.Options.Count != 0)
                .ToList().ForEach(cosmetic => cosmetic.Options.Clear());
    }
    ...
    private void HeroDefinition_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (Generate.Cache.ContainsKey(Generate.Type.Characters))
            Generate.Cache[Generate.Type.Characters]?.Cosmetics.Values.Where(cosmetic => cosmetic.Options.Count != 0)
                .ToList().ForEach(cosmetic => cosmetic.Options.Clear());
    }
    ...
    private void Nsfw_Click(object sender, RoutedEventArgs e)
    {
        ...
        Memory.Clear();
        Memory.LobbyView = new LobbyView();
        Generate.Cache.Clear();
    }
