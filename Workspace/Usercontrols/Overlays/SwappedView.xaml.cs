    ...
    private void SwappedView_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        foreach (var Swapped in Parse)
            try
            {
                ...
                NewSwappedControl.TimeStamp.Text =
                    string.Format(Languages.Read(Languages.Type.View, "SwappedView", "TimeStamp"),
                        Swapped["SwappedAt"].Value<string>());
                NewSwappedControl.AssetCount.Text = string.Format(
                    Languages.Read(Languages.Type.View, "SwappedView", "AssetCount"),
                    Swapped["AssetCount"].Value<int>());
