...
public static class Endpoint
{
    ...
    public static JToken Read(Type type)
    {
        ...
        if (!Parse.ContainsKey(type.ToString().ToLower()))
        {
            ...
            Message.DisplaySTA("Error", $"Failed to find {type} in endpoint cache.", exit: true,
                solutions: new[]
        ...
        if (Settings.Read(Settings.Type.IsDev).Value<bool>())
        {
            ...
            Parse["cosmetics"] =
                JObject.Parse(File.ReadAllText("D:\\Galaxy Swapper v2\\Backend\\API\\1.13\\Cosmetics.json"));
        }
        ...
        using (var client = new RestClient())
        {
            ...
            try
            {
                if (response.StatusCode != HttpStatusCode.OK)
                    new Exception("response did not return as status code OK");
                ...
                if (Parse["iscompressed"] is not null && Parse["iscompressed"].Value<bool>())
                {
                    ...
                    try
                    {
                        var decompressedBuffer = gzip.Decompress(Parse["compressedbuffer"].Value<string>());
                    ...
                    catch (Exception Exception)
                    {
                        ...
                        Message.DisplaySTA("Error", "Failed to decompress response from endpoint!", discord: true,
                            solutions: new[]
                            {
                                "Disable Windows Defender Firewall", "Disable any anti-virus softwares", "Turn on a VPN"
                            }, exit: true);
