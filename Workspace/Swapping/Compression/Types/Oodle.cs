namespace LilySwapper.Workspace.Swapping.Compression.Types;
...
public static class Oodle
{
    ...
    private static extern int OodleLZ_Compress(OodleFormat format, byte[] decompressedBuffer, long decompressedSize,
        byte[] compressedBuffer, OodleCompressionLevel compressionLevel, uint a, uint b, uint c, uint threadModule);

    [DllImport("oo2core_5_win64.dll")]
    private static extern int OodleLZ_Decompress(byte[] buffer, long bufferSize, byte[] outputBuffer,
        long outputBufferSize, uint a, uint b, ulong c, uint d, uint e, uint f, uint g, uint h, uint i,
        uint threadModule);
    ...
    public static byte[] Compress(byte[] buffer, OodleCompressionLevel CompressionLevel)
    {
        ...
        try
        {
            Uint = (uint)OodleLZ_Compress(OodleFormat.Kraken, buffer, buffer.Length,
                new byte[(int)(uint)buffer.Length + 274U * (((uint)buffer.Length + 262143U) / 262144U)],
                CompressionLevel, 0U, 0U, 0U, 0);
        }
        ...
        var compressedBytes = new byte[Uint + (uint)OodleLZ_Compress(OodleFormat.Kraken, buffer, buffer.Length, array,
            CompressionLevel, 0U, 0U, 0U, 0U) - (int)Uint];
        Buffer.BlockCopy(array, 0, compressedBytes, 0,
            OodleLZ_Compress(OodleFormat.Kraken, buffer, buffer.Length, array, CompressionLevel, 0U, 0U, 0U, 0U));
