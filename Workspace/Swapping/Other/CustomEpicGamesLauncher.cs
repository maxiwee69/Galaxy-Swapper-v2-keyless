...
{
    private static readonly List<byte[]> cpatterns = new()
    {
        new byte[]
        {
    ...
    {
        var EpicInstalltion = Settings.Read(Settings.Type.EpicInstalltion).Value<string>();

        if (!File.Exists(EpicInstalltion))
            throw new CustomException(
                $"Epic Games Launcher does not exist at:{EpicInstalltion}\nPlease ensure you have the corredct path for Epic Games Launcher selected!");
        if (!EpicInstalltion.CanEdit())
            throw new CustomException(
                "Failed to open stream for Epic Games Launcher!\nPlease ensure Epic Games Launcher is closed during this process.");

        Log.Information($"Opening stream for: {EpicInstalltion}");

        using (var stream = new FileStream(EpicInstalltion, FileMode.Open, FileAccess.ReadWrite))
        {
            ...
            foreach (var pattern in cpatterns)
            {
                var position = Misc.IndexOfSequence(stream, pattern, 20000000);
    ...
    public static void Revert()
    {
        ...
        const string url =
            "https://raw.githubusercontent.com/GalaxySwapperOfficial/Galaxy-Swapper-API/main/In%20Game/EpicGamesLauncher";
