...
public class Serializer
{
    ...
    public void WriteNameMaps(Writer writer)
    {
        ...
        foreach (var hash in Deserializer.Hashes) writer.Write(hash);

        foreach (var entry in Deserializer.Entires) writer.WriteBytes(new byte[] { 0, (byte)entry.Name.Length });

        foreach (var entry in Deserializer.Entires) writer.WriteBytes(Encoding.ASCII.GetBytes(entry.Name));
    }

    public void WritePad(Writer writer)
    {
        writer.Write((ulong)Deserializer.Pad.Length);
    ...
    public void WriteExportMap(Writer writer)
    {
        ...
        var ExportMap = Deserializer.ExportMap;
        for (var i = 0; i < ExportMap.Length; i++)
        {
            var fExportMapEntry = ExportMap[i];
